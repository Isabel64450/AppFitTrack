...
export default function Exercises() {
  ...
  // Fonction de chargement avec paramètres
  ...
  // Recharge quand la catégorie change (immédiat)
  useEffect(() => { loadExercises() }, [catFilter])

  // Recharge après 350ms de pause de frappe (debounce)
  useEffect(() => {
    const t = setTimeout(() => loadExercises(), 350)
    return () => clearTimeout(t)
    // Le cleanup annule le timer si l'utilisateur tape à nouveau avant 350ms
  }, [search])
  ...
  const openEdit = (ex: Exercise) => {
    setEditTarget(ex)
    setForm({ name: ex.name, category: ex.category,
    ...
    try {
      if (editTarget) {
        ...
        setExercises(exercises.map(ex =>
          ex.id === editTarget.id ? res.data.exercise : ex
        ))
