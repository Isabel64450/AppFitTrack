...
const WorkoutController = {
  ...
  async getOne(req, res) {
    try {
      ...
      // pas les séances d'un autre utilisateur (sécurité IDOR)
  ...
  async update(req, res) {
    try {
      ...
      await WorkoutModel.update(req.params.id, req.user.id, { title, date, duration, notes });// Si exercises est fourni : REMPLACER tous les exercices
  ...
  async delete(req, res) {
    try {
      const deleted = await WorkoutModel.delete(req.params.id, req.user.id);   if (!deleted) return res.status(404).json({ error: 'Workout not found.' });
